...
def plot_decision_boundary(pred_func, X, y, h=.02):   #h is the grain size
...
def build_model(X, y, hid_dim=5, act_func='tanh', grad_passes=20000, learning_rate=.01, regularization=.01, print_loss=True): 
...
#act_func is the activation function on the hidden layer, grad_passes is number of times we run through gradient descent to train weights
    ...
    Y=one_hot_encoder(y)
        ...
        #Set initial weights to 0, we'll have to learn these
    W1=np.random.randn(in_dim,hid_dim)/np.sqrt(out_dim)  #0's weight matrix from input to hidden layer
    W2=np.random.randn(hid_dim,out_dim)/np.sqrt(hid_dim) #0's weight matrix from hidden layer to output layer
    ...
    for i in xrange(grad_passes):
        ...
        if print_loss and i % 1000 == 0:
            print ("Loss after iteration %i: %f" %(i, error_function(y, model)))
            

    return model
        
        
model = build_model(Xblob, yblob)  
plot_decision_boundary(lambda x: predict(model, x), Xblob, yblob)
